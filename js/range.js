...
/*
  ...
  [constructor] CRange(root) => Return a CRange which can work on all content inside of root. Root can't be changed
  
  CRange.toString() => Return the text in the range
  [get] CRange.length => Give the length of the TEXT in the range
  
  CRange.highlight() => Make a selection of the ranged text on the root
  CRange.getBoundingClientRect() => Return a DOMRect of the bouding box enclosing the range  
  
  CRange.growAll() => Give a range including all root content
  CRange.growToLeft() => Return a range grown up to reach the first left content in root
  CRange.growToRight() => Return a range grown up to reach the last right content in root
  
  CRange.collapseToLeft() => Return a collapsed range (start = end) on the left side
  CRange.collapseToRight() => Return a collapsed range (start = end) on the right side
  
  CRange.none() => Give a null range (not a collapsed one)
  CRange.fitToSelection() => Return a range of the selected contect IN THE ROOT, or none if not FIXME COHERENCE
  
  CRange.search(selectionPattern, [prePattern], [afterPattern]) => Give a subrange of the first match findable from the regular expression
  
    CRange.merge(range) => Return a unified range, both roots must be the same node  FIMEX
  CRange.replaceText(text) => Change the text of the range
  
*/

function CRange(root, _range) {
  if (! root) throw new Error("CRange.root must be set.");
...
// Select the range
CRange.prototype.highlight = function () {
...
// ReplaceText
CRange.prototype.replaceText = function (text) {
  if (this._range) {
    var selection = window.getSelection();
    selection.removeAllRanges();
...
// Range all root up
CRange.prototype.growAll = function () {
  var rangeAll = document.createRange();
  rangeAll.selectNodeContents(this._root); // .selectNode(this._root);
  return new CRange(this._root, rangeAll);
};

// Null range
CRange.prototype.none = function () {
  return new CRange(this._root, null);
};

CRange.prototype.fitToSelection = function () {
  var selection = window.getSelection();
  ...
  */
  ...
  return new CRange(this._root, selectionRange);   
};

CRange.prototype.merge = function (range) {

  if (! (range instanceof CRange)) throw new Error("Argument of CRange.merge must be a CRange");
  ...
  if (this._root.contains(range._root)) {
    ...
    throw new Error("CRange.merge cannot be used with non-nested root ranges");
  }
  ...
  // Both empty ranges
  if (! (a || b)) {
    return new CRange(commonAncestor, null);
  }
  
  // range empty, this ranged
  else if (! b) { 
    return new CRange(commonAncestor, a.cloneRange());
  }
  
  // this empty, range ranged,
  else if (! a) {      
    return new CRange(commonAncestor, b.cloneRange());
  }
  ...
  */
  ...
  return new CRange(commonAncestor, mergedRange);
  
};

CRange.prototype.growToLeft = function () {
  ...
  if (this._range) {
   ...
   return new CRange(this._root, cursorLeftRange); 
};

CRange.prototype.growToRight = function () {
  ...
  if (this._range) {
   ...
   return new CRange(this._root, cursorRightRange); 
};

CRange.prototype.collapseToLeft = function () {
  ...
  return new CRange(this._root, collapsedLeftRange); 
};

CRange.prototype.collapseToRight = function () {
  ...
  return new CRange(this._root, collapsedRightRange); 
};

// Subrange text
CRange.prototype.search = function (selPattern, prePattern, afterPattern) {
...
// Screen absolute
CRange.prototype.getBoundingClientRect = function () {
...
CRange.prototype._getEnchainedText = function () {
  ...
  var enchainedText = {
    ...
    // Convert a text range into a CRange
    offsetTextToNode: function (subStartOffset, subEndOffset) { 
      ...
      // Create a CRange from the text offset
      ...
      return new CRange(enchainedText._root, subRange);
    }
  };
...
Object.defineProperty(CRange.prototype, "length", {
  get () {
...
/*
  Expose the same public members of CRange, but works inside of a TEXTAREA or INPUT content
...
function CTextareaRange(textBox, _selectionStart, _selectionEnd) {
...
// Convert to string
CTextareaRange.prototype.toString = function () {
...
// FIXME Length
Object.defineProperty(CTextareaRange.prototype, "length", {
  get () {
...
// Expand all
CTextareaRange.prototype.growAll = function () {
  return new CTextareaRange(this._textBox, 0, Infinity);
};

// Expand cursor selection: "The |dog is| sleeping" => "|The dog is| sleeping" or "The |dog is sleeping|"
CTextareaRange.prototype.growToLeft = function () {
  return new CTextareaRange(this._textBox, 0, this._selectionEnd);
};

CTextareaRange.prototype.growToRight = function () {
  return new CTextareaRange(this._textBox, this._selectionStart, Infinity);
};

// Compress Cursor Selection: "The |cat says| no" => "The ||cat says no" or "The cat says|| no"
CTextareaRange.prototype.collapseToLeft = function () {
  return new CTextareaRange(this._textBox, this._selectionStart, this._selectionStart);
};

CTextareaRange.prototype.collapseToRight = function () {
  return new CTextareaRange(this._textBox, this._selectionEnd, this._selectionEnd);
};

CTextareaRange.prototype.none = function () {
  return new CTextareaRange(this._textBox, undefined, undefined);
};

// Return a range of the selected text, no matter what start/end on this
CTextareaRange.prototype.fitToSelection = function () {
  return new CTextareaRange(this._textBox, this._textBox.selectionStart, this._textBox.selectionEnd);
};

// CTextareaRange.prototype.normalize 
CTextareaRange.prototype.normalize = function () {
  // A normalized range is always within range [0, max] with max <= length
  if (this._selectionStart === undefined) {
    return new CTextareaRange(this._textBox, 0, 0);
  }
  ...
  return new CTextareaRange(this._textBox, start, end);
};

CTextareaRange.prototype.highlight = function () {
...
CTextareaRange.prototype.replaceText = (function () {
...
CTextareaRange.prototype.search = function (pattern) {
  ...
  if (matched) {
    return new CTextareaRange(this._textBox,
...
CTextareaRange.prototype.merge = function (range) {

  if (! (range instanceof CTextareaRange) || ! (range._textBox.isSameNode(this._textBox))) throw new Error("Argument of CTextareaRange.merge must be a CTextareaRange with same textBox");

  if (this._selectionStart === undefined) {
    return new CTextareaRange(this._textBox, range._selectionStart, range._selectionEnd);
  }
  
  else if (range._selectionStart === undefined) {
    return new CTextareaRange(this._textBox, this._selectionStart, this._selectionEnd);
  }
...
CTextareaRange.prototype.getBoundingClientRect = (function () {
// FIXME
